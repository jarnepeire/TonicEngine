...
void LevelOne::Initialize()
{
	...
	auto pLevelText = pLevelUI->AddComponent<TextComponent>(std::make_shared<TextComponent>("Level: 1", qBertSmallFont));
	...
	auto pNextLevelObserver = std::make_shared<NextLevelObserver>(hexGridComp, "LevelTwo", nextLevelSoundId);
...
void LevelOne::ResetLevel()
{
	...
	m_pQBert->GetComponent<HexJumpComponent>()->ResetPosition();
